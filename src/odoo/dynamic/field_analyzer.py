...
class FieldAnalyzer:
    ...
    def get_field_groups(self, model_name: str) -> Dict[str, List[str]]:
        """Group fields of a model into logical categories.
        ...
        # Helper function to check if a field name contains any of the given keywords
        def contains_keywords(field_name: str, keywords: List[str]) -> bool:
            return any(keyword in field_name.lower() for keyword in keywords)

        for field_name, field_info in fields.items():
            field_type = field_info.get('type')

            # Categorize fields based on name, type, and other characteristics
            if field_name in ['name', 'display_name', 'code', 'reference', 'sequence']:
                groups["Basic Information"].append(field_name)
            elif contains_keywords(field_name, ['email', 'phone', 'mobile', 'address', 'city', 'zip', 'country', 'website']):
                groups["Contact Information"].append(field_name)
            elif field_type in ['date', 'datetime'] or contains_keywords(field_name, ['date', 'time', 'deadline', 'schedule']):
                groups["Dates and Times"].append(field_name)
            elif field_type in ['many2one', 'one2many', 'many2many'] or field_name.endswith('_id'):
                groups["Relationships"].append(field_name)
            elif field_name in ['state', 'status', 'active', 'stage_id'] or contains_keywords(field_name, ['state', 'status', 'stage']):
                groups["Status and State"].append(field_name)
            elif field_type in ['integer', 'float', 'monetary'] or contains_keywords(field_name, ['amount', 'price', 'cost', 'quantity', 'qty', 'number']):
                groups["Numeric Values"].append(field_name)
            elif field_type in ['text', 'html'] or contains_keywords(field_name, ['description', 'note', 'comment']):
                groups["Description and Notes"].append(field_name)
            elif field_name in ['id', 'create_uid', 'create_date', 'write_uid', 'write_date', '__last_update']:
                groups["System Fields"].append(field_name)
            else:
                groups["Other"].append(field_name)

        # Remove empty groups
        return {group_name: field_list for group_name, field_list in groups.items() if field_list}