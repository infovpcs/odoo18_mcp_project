...
class OdooDocsRetriever:
            ...
            if self.engine.load_index_and_documents():
                ...
                if self.engine.model and self.engine.index:
                    ...
                    if test_embedding.shape[0] != self.engine.index.d:
                        logger.warning(f"Model dimension ({test_embedding.shape[0]}) doesn't match index dimension ({self.engine.index.d})")
    ...
    def retrieve(
        ...
        try:
            ...
            try:
                results = self.engine.search(processed_query, k=max_results * 3)  # Get more results for filtering
            ...
            if "mail" in query.lower() or "email" in query.lower() or "mailing" in query.lower():
                boost_keywords.extend(["mail", "email", "mailing", "message", "notification"])
                ...
                for result in filtered_results:
                    ...
                    for keyword in boost_keywords:
                        if keyword in text_lower:
                            ...
                            boost_score += 0.05 * min(occurrences, 5)  # Cap at 5 occurrences
            ...
            logger.info(f"Retrieved {len(limited_results)} results for query: '{query}'")
            for i, result in enumerate(limited_results):
                logger.info(f"Result {i+1}: Score={result['score']:.2f}, Source={result['metadata'].get('file_name', 'Unknown')}")
